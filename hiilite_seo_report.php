...
while($reports->have_posts()):
	...
	$propertyId = (int)get_post_meta( get_the_ID(), 'hiiseo_ga_property_id', true ) != null?((int)get_post_meta( get_the_ID(), 'hiiseo_ga_property_id', true ) - 1):0;
	...
	function getFirstprofileId(&$analytics, $accountId, $propertyId) {
	  ...
	  // Get the list of accounts for the authorized user.
		 if(isset($accountId)){
			...
			if (count($properties->getItems()) > 0) {
			  ...
			  $firstPropertyId = $items[$propertyId]->getId();
	 		  //echo '<h1>'.$firstPropertyId.'</h1>';
	...
	$profile = getFirstProfileId($analytics, $accountId, $propertyId);
	//$results = getResults($analytics, $profile);
	//
	//echo '<pre>';
	//print_r($profile);
	$site_name = $analytics->management_webproperties->listManagementWebproperties($accountId)->items[$propertyId]->name;
	$site_url = $analytics->management_webproperties->listManagementWebproperties($accountId)->items[$propertyId]->websiteUrl;
	//echo '</pre>';
	?>
	<style>
	@font-face {
